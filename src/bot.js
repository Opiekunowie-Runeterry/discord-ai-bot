...
async function replySplitMessage(replyMessage, content, image) {
  ...
  if (image) {
    await replyMessage.channel.send({ files: [image] });
  }
...
client.on(Events.MessageCreate, async (message) => {
  ...
  try {
    ...
    try {
      ...
      image = Buffer.from(stableDiffusionResponse.images[0], "base64");
    ...
    const replyMessageIDs = (
      await replySplitMessage(message, `${prefix}${responseText}`, image)
    ).map((msg) => msg.id);
